...
def one_loop():
    ...
    rospy.set_param('crop_max_y', 0.275)
    ...
    rospy.set_param('ec_max_cluster_size', 30000)
    rospy.loginfo("set!")
    ...
    reader = PointSReader()
    sub = rospy.Subscriber("/segment_cloud", Marker, reader.callback)
    rospy.set_param('ec_min_cluster_size', 3000)
    rospy.set_param('ec_max_cluster_size', 25000)
    rospy.sleep(2)
    while reader.pose == None:
        rospy.sleep(0.1)
    rospy.loginfo(ps1)
    ...
    # transformed_pose.position.z -= 0.02
    ps1.pose = transformed_pose
